...
namespace IRasRag.Infrastructure.Persistence
{
    public class UnitOfWork : IUnitOfWork
    {
        ...
        private static readonly Dictionary<string, string> UniqueConstraintMessages = new()
        {
            ...
            ["ix_control_devices_master_board_id_pin_code"]                           = "Mã pin này đã được sử dụng trên board này.",
            ["ix_growth_stages_name"]                                                 = "Tên giai đoạn tăng trưởng đã tồn tại.",
            ["ix_job_types_name"]                                                     = "Tên loại công việc đã tồn tại.",
            ["ix_job_control_mappings_job_id_control_device_id"]                      = "Thiết bị này đã được gán cho công việc này.",
            ["ix_sensor_logs_sensor_id_period_start"]                                 = "Đã tồn tại log cảm biến cho khoảng thời gian này.",
            ["ix_species_stage_configs_species_id_growth_stage_id"]                   = "Đã tồn tại cấu hình cho loài và giai đoạn tăng trưởng này.",
            ["ix_species_thresholds_species_id_growth_stage_id_sensor_type_id"]       = "Đã tồn tại ngưỡng cho loài, giai đoạn và loại cảm biến này.",
            ["ix_user_farms_user_id_farm_id"]                                         = "Người dùng đã được gán vào trại này.",
            ["ix_alerts_fish_tank_id_farming_batch_id_sensor_type_id"]                = "Đã tồn tại cảnh báo đang mở cho bể, lứa nuôi và loại cảm biến này.",
            ["ix_alerts_fish_tank_id_sensor_type_id"]                                 = "Đã tồn tại cảnh báo đang mở cho bể và loại cảm biến này.",
        };

        private static Exception ResolveDbConflict(DbUpdateException ex)
        {
            if (ex.InnerException is not PostgresException pgEx)
                return new DbConflictException(FallbackFromMessage(ex.InnerException?.Message ?? ex.Message));

            return pgEx.SqlState switch
            {
                PostgresErrorCodes.UniqueViolation      => new DbConflictException(ResolveUniqueViolation(pgEx)),
                PostgresErrorCodes.ForeignKeyViolation  => ResolveForeignKeyViolation(pgEx),
                _                                       => new DbConflictException("Lỗi ràng buộc cơ sở dữ liệu. Vui lòng kiểm tra lại dữ liệu."),
            };
        }
        ...
        private static Exception ResolveForeignKeyViolation(PostgresException pgEx)
        {
            ...
            if (detail.Contains("is not present in table", StringComparison.OrdinalIgnoreCase))
            {
                ...
                var referenced = ExtractTableFromDetail(detail, @"is not present in table ""([^""]+)""");
            ...
            var referencingTable = ExtractTableFromDetail(detail, @"still referenced from table ""([^""]+)""");
