...
namespace IRasRag.Application.Services.Implementations
{
    public class AuthService : IAuthService
    {
        ...
        public AuthService(IUnitOfWork unitOfWork, ILogger<AuthService> logger, IPasswordHasher hasher, IJwtService jwtService)
        {
        ...
        public async Task<Result<string>> Login(LoginRequest request)
        {
            ...
            var user = await _unitOfWork.GetRepository<User>().FirstOrDefaultAsync(u => normalizedUserName == u.UserName);

            var isValidUser = user != null && _hasher.VerifyPassword(request.Password, user.PasswordHash);

            if (!isValidUser)
            {
                _logger.LogWarning("Invalid login attempt for user: {UserName}", request.UserName);
                return Result<string>.Failure("Sai tài khoản hoặc mật khẩu.", ResultType.Unauthorized);
            }
        ...
        {
            var normalizedUserName = request.UserName.Trim().ToLower();
            var isUserNameTaken = await _unitOfWork.GetRepository<User>()
            ...
            {
                return Result<string>.Failure("Mật khẩu và xác nhận mật khẩu không khớp.", ResultType.BadRequest);
            }
